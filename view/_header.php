<!DOCTYPE html>

<html>
    <head>
        <meta charset="UTF-8">
        <link href="styles/bootstrap/css/bootstrap.css" rel="stylesheet" type="text/css"/>
        <link href="styles/css/estilo.css" rel="stylesheet" type="text/css"/>
        <!--<link href="styles/mdl/material.min.css" rel="stylesheet" type="text/css"/>-->
        <!--<script src="styles/mdl/material.min.js" type="text/javascript"></script>-->
        <script src="styles/js/jquery.js" type="text/javascript"></script>
        <script src="styles/js/Chart.js-master/Chart.js" type="text/javascript"></script>
        <script src="styles/js/amcharts/amcharts/amcharts.js" type="text/javascript"></script>
        <script src="styles/js/amcharts/amcharts/serial.js" type="text/javascript"></script>
        <script src="styles/js/amcharts/amcharts/themes/light.js" type="text/javascript"></script>
        <script src="styles/bootstrap/js/bootstrap.min.js" type="text/javascript"></script>
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        
        <title>Medição Residêncial</title>
    </head>
    <body>
